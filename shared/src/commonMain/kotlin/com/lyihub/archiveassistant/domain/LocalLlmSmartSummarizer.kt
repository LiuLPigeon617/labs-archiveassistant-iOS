...
class LocalLlmSmartSummarizer(private val engine: LocalLlmEngine) : SmartSummarizer {
  ...
  override suspend fun summarize(
    ...
    val output =
      engine
        .generate(promptFor(request, normalizedInput, sixMinistryTopics), maxTokens = 768)
    ...
    return runCatching { parseResult(output) }
      .getOrElse { SmartSummarizeResult.Failure("本地模型返回格式无效，请重试") }
  }
  ...
  private fun parseResult(output: String): SmartSummarizeResult.Success {
    ...
    val contentType =
      requireNotNull(contentTypeValue(parsed.contentType)) { "Invalid contentType" }
    val documentFormat =
...
"""
    ...
    return """
        ...
        {"topicId":"六部ID","contentType":"WEB_ARTICLE","title":"简洁标题","summary":"一句话摘要","sourceUrl":"来源URL或空字符串","documentFormat":"PDF"}
    """
      .trimIndent()
  }
